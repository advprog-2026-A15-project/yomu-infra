# seed-bacaan.ps1 — Inject sample bacaan data into service-learning
# Run: powershell -ExecutionPolicy Bypass -File .\seed-bacaan.ps1

$BASE_URL = "http://localhost:8082/api/learning/bacaan"
...
$bacaanList = @(
    ...
    @{
        ...
        content  = @"
Di sebuah desa kecil di kaki Gunung Merapi, hiduplah seorang anak bernama Aditya. Setiap malam, ia bermimpi terbang melintasi awan-awan putih yang berkilauan di bawah cahaya bulan. Mimpi-mimpi itu begitu nyata sehingga ia bisa merasakan hembusan angin di wajahnya.
...
"@
    ...
    @{
        ...
        content  = @"
Perpustakaan Kota Surabaya yang dibangun pada tahun 1920 menyimpan ribuan buku dari berbagai era. Namun, di balik rak-rak berdebu di lantai tiga, tersembunyi sebuah ruangan yang tidak pernah dikunjungi siapa pun selama puluhan tahun.
...
"@
    ...
    @{
        ...
        content  = @"
Membaca bukan sekadar aktivitas menyerap informasi — ia adalah latihan kompleks bagi otak yang melibatkan berbagai jaringan saraf secara simultan. Penelitian neurosains modern telah mengungkapkan bahwa membaca memiliki dampak transformatif pada struktur dan fungsi otak.
...
"@
    ...
    @{
        ...
        content  = @"
Metode Pomodoro dikembangkan oleh Francesco Cirillo pada akhir tahun 1980-an saat ia masih menjadi mahasiswa yang kesulitan berkonsentrasi. Nama metode ini diambil dari timer dapur berbentuk tomat (pomodoro dalam bahasa Italia) yang ia gunakan.
...
"@
    ...
    @{
        ...
        content  = @"
Lubang hitam (black hole) adalah salah satu objek paling misterius dan menakjubkan di alam semesta. Mereka terbentuk ketika bintang raksasa — dengan massa minimal 20 kali massa Matahari — kehabisan bahan bakar nuklir dan runtuh ke dalam dirinya sendiri akibat gravitasi yang sangat kuat.
...
"@
    ...
    @{
        ...
        content  = @"
Fotosintesis adalah proses biokimia fundamental yang mengubah energi cahaya matahari menjadi energi kimia, menjadi dasar dari hampir seluruh rantai makanan di Bumi. Tanpa fotosintesis, kehidupan seperti yang kita kenal tidak akan mungkin ada.
...
"@
    ...
    @{
        ...
        content  = @"
Kerajaan Majapahit (1293-1527 M) merupakan kerajaan Hindu-Buddha terbesar yang pernah berdiri di Nusantara. Didirikan oleh Raden Wijaya setelah jatuhnya Kerajaan Singhasari, Majapahit tumbuh menjadi kekuatan maritim yang menguasai sebagian besar kepulauan Asia Tenggara.
...
"@
    ...
    @{
        ...
        content  = @"
Tanggal 17 Agustus 1945 menandai titik balik dalam sejarah bangsa Indonesia — hari ketika Soekarno dan Mohammad Hatta memproklamasikan kemerdekaan Indonesia, mengakhiri lebih dari tiga abad penjajahan.
...
"@
    ...
    @{
        ...
        content  = @"
Kecerdasan Buatan (Artificial Intelligence/AI) telah berkembang dari konsep fiksi ilmiah menjadi teknologi yang mengubah hampir setiap aspek kehidupan modern. Dari asisten virtual di ponsel hingga diagnosa medis, AI hadir di mana-mana.
...
"@
    ...
    @{
        ...
        content  = @"
Blockchain adalah teknologi buku besar terdistribusi (distributed ledger) yang memungkinkan pencatatan transaksi secara transparan, aman, dan tidak dapat diubah. Pertama kali diperkenalkan pada tahun 2008 oleh sosok misterius bernama Satoshi Nakamoto sebagai fondasi Bitcoin.
...
"@
    ...
    @{
        ...
        content  = @"
Batik adalah seni pewarnaan kain menggunakan teknik perintang warna (wax-resist dyeing) yang telah dipraktikkan di Indonesia selama berabad-abad. Pada tanggal 2 Oktober 2009, UNESCO mengakui batik Indonesia sebagai Warisan Kemanusiaan untuk Budaya Lisan dan Nonbendawi (Intangible Cultural Heritage of Humanity).
...
"@
    ...
    @{
        ...
        content  = @"
Gamelan adalah ansambel musik tradisional Indonesia yang terdiri dari berbagai instrumen perkusi, terutama metalofon, gong, dan drum. Kata 'gamelan' berasal dari bahasa Jawa 'gamel' yang berarti memukul atau menabuh.
...
Write-Host "`n=== Seeding Bacaan Data ===" -ForegroundColor Cyan
Write-Host "Target: $BASE_URL`n"

foreach ($bacaan in $bacaanList) {
    $json = $bacaan | ConvertTo-Json -Depth 3
    try {
        $response = Invoke-WebRequest -Uri $BASE_URL -Method POST -Body ([System.Text.Encoding]::UTF8.GetBytes($json)) -ContentType "application/json; charset=utf-8" -UseBasicParsing -TimeoutSec 15
        $result = $response.Content | ConvertFrom-Json
        Write-Host "[OK] $($bacaan.category) - $($bacaan.title) (ID: $($result.id))" -ForegroundColor Green
        $success++
    }
    catch {
        Write-Host "[FAIL] $($bacaan.category) - $($bacaan.title): $($_.Exception.Message)" -ForegroundColor Red
        $failed++
    }
}
